  ...
  var _styles = {
    ...
    'default': {
      ...
      thumbnail: {
        ...
        wrap: '<div style="margin-bottom: 0; text-align: center;"></div>', 
    ...
    'jquery-ui': {
      ...
      thumbnail: {
        ...
        wrap: '<div style="padding: 0; margin-bottom: 0; text-align: center" class="ui-widget ui-widget-content ui-helper-clearfix ui-corner-all"></div>', 
...
$('.patientLogin').click(function(e){
	e.preventDefault();
	$('#patientLoginPanel').show();
});
$('.providerLogin').click(function(e){
	e.preventDefault();
       
	$('#providerLoginPanel').show();
});
$(".ChangeLogoLink").click(function(){
    
    	$('#providerChangeLogoPanel').show();
});

$('.popUpPanel .popOverlay,.popUpPanel .closeIcon').click(function(){
	$('.popUpPanel').hide();
});
});
...
function checkProviderLogin(){
  $.ajax({
      url: BASE_URL+'provider/login',
      method:'post',
      data:{
        '_token'    :   CSRF_TOKEN,
        'email'     :   $("input[name=login_provider_email]").val(),
        'password'  :   $("input[name=login_provider_password]").val()
      },
      dataType:'json',
      success:function(msg){
        if (msg.code=='0') {
            $('.loginErrmsg').html(msg.msg);
        }else{
            $('.loginErrmsg').empty();
        }
        if (msg.code == '1') {
            window.location.href=BASE_URL+'provider/dashboard';
        }
      }
    })    
}
$(function(){
$('.providerLoginInput').keypress(function(e) {
    if(e.which == 13) {
        checkProviderLogin();
    }
});
